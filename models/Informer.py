...
class Informer(STBase):
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len, 
                ...
                output_attention = False, distil=True, mix=True, **kwargs):
    ...
    def _process_one_batch(self, batch):
        ...
        # decoder input
        dec_inp = torch.zeros([y.shape[0], self.pred_len, y.shape[-1]]).type_as(x)
        dec_inp = torch.cat([y[:,:self.label_len,:], dec_inp], dim=1).type_as(x)
