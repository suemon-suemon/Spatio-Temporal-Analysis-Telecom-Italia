...
class LightSTGCN(STBase):
    def __init__(self,
        ...
        self.cheb_conv = cheb_conv(in_channels=1, out_channels = 8,
                                  K = cheb_k, Adj=adj_mx)  # Chebyshev图卷积

        # Time Convolution (Conv2d for (N,T) convolution)
        self.time_conv = nn.Conv2d(in_channels = 8, out_channels=feature_len,
                                   kernel_size=(1, 3),
...
if __name__ == '__main__':
    model = LightSTGCN(close_len=32, pred_len=6, feature_len=64, cheb_k=3)
    x = torch.randn(32, 400, 1, 32)
    y = model(x)
    print(y.shape)