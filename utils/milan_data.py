def get_indexes_of_train(format, time_level, out_start_idx, close_len, period_len, trend_len=0, *, pred_len=1):
    ...
    if format == 'default':
        ...
        if period_len > 0:
            indices += [out_start_idx-(i+1)*TIME_STEPS_OF_DAY+(pred_len-j-1) 
                        for j in range(pred_len) for i in range(period_len)]
        if trend_len > 0:
            indices += [out_start_idx-(i+1)*TIME_STEPS_OF_DAY*7+(pred_len-j-1) 
                        for j in range(pred_len) for i in range(trend_len)]
    elif format == 'sttran':
        if period_len > 0:
            indices += [out_start_idx-(i+1)*TIME_STEPS_OF_DAY-j 
